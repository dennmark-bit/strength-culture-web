  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center bg-darkbg overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-4xl mx-auto px-6">
        <p className="text-gold text-xs uppercase tracking-[0.4em] mb-6">Central, Hong Kong</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a
            ...
            className="bg-gold text-black px-8 py-4 font-black uppercase tracking-widest text-sm hover:bg-yellow-400 transition"
          >
            Book a Free Trial
          </a>
          <Link
            href="#classes"
            className="border border-white/30 text-white px-8 py-4 font-bold uppercase tracking-widest text-sm hover:border-gold hover:text-gold transition"
          >
            See Classes
          </Link>
        </div>
        <div className="mt-16 grid grid-cols-3 gap-8 max-w-md mx-auto">
          {[['6AM', 'Earliest Class'], ['10PM', 'Latest Weekday'], ['Central', 'Li Yuen St West']].map(([val, label]) => (
            <div key={label} className="text-center">
              <p className="text-gold text-2xl font-black">{val}</p>
